...
class Operator:
    ...
    def change_satellite_coordinates(
        self, satellite: Satellite, delta_lat: float, delta_long: float
    ):
        lat, long = satellite.coordinates
        new_lat = get_valid_lat(lat + delta_lat)
        new_long = get_valid_long(long + delta_long)
        satellite.coordinates = (new_lat, new_long)

    def change_group_coordinates(
        self, group: GroupOfSatellites, delta_lat: float, delta_long: float
    ):
