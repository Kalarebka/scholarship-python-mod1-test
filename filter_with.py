...
def filter_with(data, keyword):
    ...
    if not has_nested_list_or_dict(data):
        if isinstance(data, list):
            ...
            return dict(
                filter(lambda item: contains_keyword(item[1], keyword), data.items())
            )
    # Have to go deeper & data is a list?
    if isinstance(data, list):
        new_data = [
            filter_with(item, keyword) if is_list_or_dict(item) else item
            for item in data
        ]
    ...
    # Have to go deeper & data is a dict?
        ...
        new_data = {
            key: filter_with(value, keyword) if is_list_or_dict(value) else value
            for key, value in data.items()
        }
        return dict(
            filter(lambda item: contains_keyword(item[1], keyword), new_data.items())
        )
